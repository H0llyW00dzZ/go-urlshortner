...
func CreateDatastoreClient(ctx context.Context, config *Config) (*Client, error) {
	...
	if err != nil {
		...
		config.Logger.Error("🆘  ⚠️  Failed to create client", logFields...)
		return nil, err
	}
...
func SaveURL(ctx context.Context, client *Client, url *URL) error {
	...
	if err != nil {
		...
		Logger.Error("🆘  ⚠️  Failed to save URL", zap.Error(err))
		return err
	}
	return nil
}
...
func UpdateURL(ctx context.Context, client *Client, id string, newURL string) error {
	...
	if err != nil {
		Logger.Error("🆘  ⚠️  Failed to update URL", zap.String("id", id), zap.Error(err))
		return err
	}
...
func DeleteURL(ctx context.Context, client *Client, id string) error {
	...
	if err != nil {
		...
		Logger.Error("🆘  ⚠️  Failed to delete URL", zap.String("id", id), zap.Error(err))
		return err
	}
	return nil
}
